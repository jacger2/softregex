...
class SelfCriticalTrainer(object):
    ...
    """
    def __init__(self, expt_dir='experiment_sc', loss=PositiveLoss(), batch_size=64,
                 random_seed=None,
                 checkpoint_every=100, print_every=100, output_vocab=None):
    ...
    def _train_batch(self, input_variable, input_lengths, target_variable, model, teacher_forcing_ratio):
        ...
        loss.eval_batch(log_tensor, output_tensor, target_variable, self.output_vocab)
            
        #print(len(decoder_outputs), len(target_variable))
        #print(decoder_outputs[0].shape, decoder_outputs[0], target_variable[0])
        #print(decoder_outputs[0].contiguous().view(32,-1), decoder_outputs[0].contiguous().view(32,-1).shape)
    ...
    def _train_epoches(self, data, model, n_epochs, start_epoch, start_step,
        ...
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            if dev_data is not None:
                #dev_loss = self.evaluator.evaluate_reward(model, dev_data, self.output_vocab)
    ...
    def train(self, model, data, num_epochs=1,
        ...
        self._train_epoches(data, model, num_epochs,
                            start_epoch, step, dev_data=dev_data, teacher_forcing_ratio=teacher_forcing_ratio)
        return model
